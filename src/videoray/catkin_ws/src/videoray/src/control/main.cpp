...
int main(int argc, char **argv)
{
     ...
     while (ros::ok()) {
       ...
       //ROS_INFO( "************* Control Node Running *************" );

          if (joystick_enabled_) {
            //ROS_INFO("joystick enabled********");
               if (rising_edge(button_[0], button_prev_[0])) {
                    if (!camera_config_) {
               ...
               /*if(button_[13]){
                 comm.set_cam_cmd(VideoRayComm::Disable);
               }*/
               ...
               tilt_ += 0.5*(button_[4]-button_[6]);//invert_sign(normalize(axis_[5], -32767, 32767, -1, 1));
               ...
               if (!camera_config_) {
                    ...
                    if (rising_edge(button_[4], button_prev_[4])) {
                         comm.set_cam_cmd(VideoRayComm::Arrow_Up);
                    } else if (rising_edge(button_[6], button_prev_[6])) {
                         comm.set_cam_cmd(VideoRayComm::Arrow_Down);
                    } else if (rising_edge(button_[7], button_prev_[7])) {
                         comm.set_cam_cmd(VideoRayComm::Arrow_Left);
                    } else if (rising_edge(button_[5], button_prev_[5])) {
               ...
               if (rising_edge(button_[15], button_prev_[15])) {
                    std_msgs::Bool msg;
                    if (enable_log_) {
          ...
          //ROS_INFO("Control main running  after syllo node spin****");
          if (status != VideoRayComm::Success) {
               cout << "Exec Transfer Error!" << endl;
               ROS_INFO("Exec Transfer Error!******");
          }
